...
# slope = slope of tangent line;
# dx    = length of line-segment (for the tangent);
#' @export
test.circle.p2s = function(p1 = c(1,8), p2=c(5,5), slope = 1/12,
		dx=c(-5, 5), col = 1, col.line = "#32D048", add = FALSE) {
...
# (x, y) = coordinates of 3 points;
#' @export
test.circle.p3 = function(x, y, msg = "",
		col.poly = "blue", col.points = "red") {
	cc = circle.p3(x, y);
	if(is.NaN1(cc$r)) {
	...
	polygon(x, y, border = col.poly);
	points(x, y, col = col.points);
}

###
#' @export
test.circle.p3Tests = function() {
